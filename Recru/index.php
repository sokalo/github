  ...
  var formNameValue = 0;
    $("body").on("click", ".add", function(e) {
        ...
        mytarget.after($(theCloneHtml).append('<div class="input-row"> <table>' +
                                   '<tr>' +
                                   '<td>' +
                                   '</br><span><input  type="text" name="education[]"></span>' +
                                   '</td>' +
                                   '<td>' +
                                   '</br><span><input class="to inp" type="text" name="education[]"></span>' +
                                   '</td>' +
                                   '<td>' +
                                   '</br><span><input class="from inp" type="text" name="education[]"></span>' +
                                   '</td>' +
                                   '<td>' +
                                   '</br><span><input  type="text" name="education[]" ></span>' +
                                   '</td>' +
...
$(function() {
  var formNameIncrement1 = 0;
  var formNameValue1 = 0;
    $("body").on("click", ".addforeign", function(f) {
        var mytarget1 = $(this).closest('.clonableforeign').find('.cloneforeign:last');
        var myparent1 = $(this).closest('.clonableforeign');
        var theCloneHtml1 = '<div class="cloneforeign" id="cloneIDforeign' + formNameIncrement1 + '">';
        var theCloneId1 = 'cloneIDforeign' + formNameIncrement1;
        myparent1.addClass('data-added-foreign');
        mytarget1.after($(theCloneHtml1).append('<div class="input-row-foreign"> <table>' +
                                   '<tr>'+
                                           '<td>'+
                                            '<span><input  type="text" name="foreignlanguage[]"></span>'+
                                            '</td>'+
                                            '<td>'+
                                            '<br><span> <select name="foreignlanguage[]" style="width:150px;">'+
                                            ...
                                            '</td>'+
                                            '<td>'+
                                            '<span><br> <select name="foreignlanguage[]">'+
                                                '<option >Beginner</option>'+
                                                '<option>Conversational</option>'+
                                                '<option>Business</option>'+
                                                '<option>Fluent</option>'+ 
                                        '</select></span>'+
                                            '</td>'+
                                            '<td>'+
                                                '<span ><select name="foreignlanguage[]">'+
                                                '<option >Beginner</option>'+
                                                '<option>Conversational</option>'+
                                                '<option>Business</option>'+
                                                '<option>Fluent</option>'+ 
                                        ...
                                        '</tr>'+
                            '</table></div>'));
        $(theCloneId1).hide().fadeIn('slow');
        formNameIncrement1++;
                f.preventDefault();
    });

     $("body").on("click", ".deleteforeign", function(f) {
        var mynum = $(this).closest('.clonableforeign').find('.cloneforeign').length;
        var mytarget = $(this).closest('.clonableforeign').find('.cloneforeign:last');
        var myparent = $(this).closest('.clonableforeign');
        if (mynum === 1) {
            myparent.removeClass('data-added-foreign')
        }
        mytarget.fadeOut();
        mytarget.remove();
        f.preventDefault();
...
// add for working experiences

$(function() {
  var formNameIncrementw = 0;
  var formNameValuew = 0;
    $("body").on("click", ".addwork", function(w) {
        var mytargetw = $(this).closest('.clonablework').find('.clonework:last');
        var myparentw = $(this).closest('.clonablework');
        var theCloneHtmlw = '<div class="clonework" id="cloneIDwork' + formNameIncrementw + '">';
        var theCloneIdw = 'cloneIDwork' + formNameIncrementw;
        myparentw.addClass('data-added-work');
        mytargetw.after($(theCloneHtmlw).append('<div class="input-row-work"> <table>' +
                                        '<tr>'+
                                           '<td>'+
                                            '<span><input  type="text" name="workexperience[]"></span>'+
                                            '</td>'+
                                            '<td>'+
                                            '<span><input  type="text" name="workexperience[]"></span>'+
                                            '</td>'+
                                        ...
                                        '</tr>'+
                            '</table></div>'));
        $(theCloneIdw).hide().fadeIn('slow');
        formNameIncrementw++;
                w.preventDefault();
    });

     $("body").on("click", ".deletework", function(w) {
        var mynum = $(this).closest('.clonablework').find('.clonework').length;
        var mytarget = $(this).closest('.clonablework').find('.clonework:last');
        var myparent = $(this).closest('.clonablework');
        if (mynum === 1) {
            myparent.removeClass('data-added-work')
        }
        mytarget.fadeOut();
        mytarget.remove();
        w.preventDefault();
...
//add references
$(function() {
  var formNameIncrementr = 0;
  var formNameValuer = 0;
    $("body").on("click", ".addreference", function(r) {
        var mytargetr = $(this).closest('.clonablereference').find('.clonereference:last');
        var myparentr = $(this).closest('.clonablereference');
        var theCloneHtmlr = '<div class="clonereference" id="cloneIDref' + formNameIncrementr + '">';
        var theCloneIdr = 'cloneIDwork' + formNameIncrementr;
        myparentr.addClass('data-added-reference');
        mytargetr.after($(theCloneHtmlr).append('<div class="input-row-reference"> <table>' +
                                        '<tr>'+
                                           '<td>'+
                                            '<span><input  type="text" name="reference[]"></span>'+
                                            '</td>'+
                                            '<td>'+
                                            '<span><input  type="text" name="reference[]"></span>'+
                                            '</td>'+
                                        ...
                                        '</tr>'+
                            '</table></div>'));
        $(theCloneIdr).hide().fadeIn('slow');
        formNameIncrementr++;
                r.preventDefault();
    });

     $("body").on("click", ".deletereference", function(r) {
        var mynum = $(this).closest('.clonablereference').find('.clonereference').length;
        var mytarget = $(this).closest('.clonablereference').find('.clonereference:last');
        var myparent = $(this).closest('.clonablereference');
        if (mynum === 1) {
            myparent.removeClass('data-added-reference')
        }
        mytarget.fadeOut();
        mytarget.remove();
                r.preventDefault();
...
<div class="career-detail">
	...
	<div class="row">
		...
		<form class="commentForm" action="application_action2.php" method="post" enctype="multipart/form-data">
		...
		<li class="career-field">
			...
			<li class="career-field">
             <fieldset class="clonableforeign">
               <legend>Add The Foreign Language Here</legend>
                   <div class="cloneforeign">
                      <div class="input-row-foreign">
                         <table id="dataTable1">
                                       <tr>
                                           <td>
                                             <label>Language</label><span><input  type="text" name="foreignlanguage[]"></span>
                                            </td>
                                            <td>
                                            <label>Speaking</label><br><span> <select name="foreignlanguage[]">
                                                <option >Beginner</option>
                                                <option>Conversational</option>
                                                <option>Business</option>
                                                <option>Fluent</option> 
                                            ...
                                            <td>
                                            <label>Reading</label><span><br> <select name="foreignlanguage[]">
                                                <option >Beginner</option>
                                                <option>Conversational</option>
                                                <option>Business</option>
                                                <option>Fluent</option> 
                                            ...
                                            <td>
                                                <label >Writing</label><span ><select name="foreignlanguage[]">
                                                <option >Beginner</option>
                                                <option>Conversational</option>
                                                <option>Business</option>
                                                <option>Fluent</option> 
                        ...
                        <div>
                              <button class="deleteforeign">Delete last row</button>
                              <button class="addforeign">add another row</button>
			...
			<li class="career-field">
              <fieldset class="clonablework">
               <legend>Add Working Experiences Here</legend>
                   <div class="clonework">
                      <div class="input-row-work">
                         <table id="dataTable1">
                                       <tr>
                                           <td>
                                             <label style="width:150px;">Company Name</label><br><span><input  type="text" name="workexperience[]"></span>
                                            </td>
                                            <td>
                                            <label style="width:150px;">Latest Position</label><br><span><input  type="text" name="workexperience[]"></span>
                                            </td>
                                            <td>
                                            <label>Period</label><br> <span><input  type="text" name="workexperience[]"></span>
                                            </td>
                                         
                                            <td>
                                                <label style="width:150px;">Latest Salary</label><span ><input  type="text" name="workexperience[]"></span>
                                            </td>
                                             <td>
                                                <label style="width:150px;">Reason For Leaving</label><span ><input  type="text" name="workexperience[]"></span>
                                            </td>
                                        </tr>
                        ...
                        <div>
                              <button class="deletework">Delete last row</button>
                              <button class="addwork">add another row</button>
                          <!--      <input type="button" value="Add Schedule" onClick="addRow('dataTable1')" /> -->
                        </div>
                </fieldset>
			...
			<li class="career-field">
             <fieldset class="clonablereference">
               <legend>Add References Here</legend>
                   <div class="clonereference">
                      <div class="input-row-reference">
                         <table id="dataTable1">
                                       <tr>
                                           <td>
                                             <label style="width:150px;">Name</label><br><span><input  type="text" name="reference[]"></span>
                                            </td>
                                            <td>
                                            <label style="width:150px;">Relationship</label><br><span><input  type="text" name="reference[]"></span>
                                            </td>
                                            <td>
                                            <label style="width:150px;">Phone/Address</label><br> <span><input  type="text" name="reference[]"></span>
                                            </td>
                                         
                                            <td>
                                                <label>Company</label><span ><input  type="text" name="reference[]"></span>
                                            </td>
                                             <td>
                                                <label >Email</label><span ><input  type="text" name="reference[]"></span>
                                            </td>
                                        </tr>
                        ...
                        <div>
                              <button class="deletereference">Delete last row</button>
                              <button class="addreference">add another row</button>
                          <!--      <input type="button" value="Add Schedule" onClick="addRow('dataTable1')" /> -->
                        </div>
            </fieldset>
        	</li>
